...
export function extractSignatureFields(settings: UserSettings | null): Partial<SignatureFields> {
  return {
...
export const SIGNATURE_DEFAULTS = {
  ...
  whatsapp: '+91 9895741814',
} as const

export function renderSignature(fields: Partial<SignatureFields> & { name: string; email: string }): string {
  const resolved: Record<string, string> = {
    name: escapeHtml(fields.name),
    designation: escapeHtml(fields.designation || SIGNATURE_DEFAULTS.designation),
    department: escapeHtml(fields.department || SIGNATURE_DEFAULTS.department),
    company: escapeHtml(fields.company || SIGNATURE_DEFAULTS.company),
    email: escapeHtml(fields.email),
    phone: fields.phone ? escapeHtml(fields.phone) : '',
    whatsapp: escapeHtml(fields.whatsapp || SIGNATURE_DEFAULTS.whatsapp),
  }
